...
chain. Pick a project from the dropdown, then **Start** for the training main
flow (`grill-with-docs` → `to-spec` → `to-tickets` → `implement` → `code-review`),
or click an **Alternatives** skill for a one-shot (ask-matt, triage, research,
…). The dial keeps polling Cursor usage on its own schedule.
